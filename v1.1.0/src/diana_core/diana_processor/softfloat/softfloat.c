...
float32 int32_to_float32(DI_INT32 a, float_status_t *status)
{
    int zSign = (a < 0);
    if (a == 0) return 0;
    if (a == (DI_INT32) 0x80000000) return packFloat32(1, 0x9E, 0);
...
float64 int32_to_float64(DI_INT32 a)
{
    int zSign;
    DI_UINT32 absA;
    int shiftCount;
...
float32 int64_to_float32(DI_INT64 a, float_status_t *status)
{
    int zSign = 0;
    DI_UINT64 absA = 0;
    int shiftCount = 0;
    ...
    if (0 <= shiftCount) {
        return packFloat32(zSign, 0x95 - shiftCount, (DI_UINT32)(absA<<shiftCount));
    }
    else {
        shiftCount += 7;
        if (shiftCount < 0) {
        ...
        return roundAndPackFloat32(zSign, 0x9C - shiftCount, (DI_UINT32) absA, status);
    }
}
...
float64 int64_to_float64(DI_INT64 a, float_status_t *status)
{
    int zSign = 0;
    if (a == 0) return 0;
    if (a == (DI_INT64) DI_CONST64(0x8000000000000000)) {
...
float32 uint32_to_float32(DI_UINT32 a, float_status_t *status)
{
    if (a == 0) return 0;
...
float64 uint32_to_float64(DI_UINT32 a)
{
   int shiftCount = 0;
   DI_UINT64 zSig = 0;
...
float32 uint64_to_float32(DI_UINT64 a, float_status_t *status)
{
    int shiftCount = 0;
    if (a == 0) return 0;
    shiftCount = countLeadingZeros64(a) - 40;
    if (0 <= shiftCount) {
        return packFloat32(0, (DI_INT16)(0x95 - shiftCount), (DI_UINT32)(a<<shiftCount));
    }
    else {
        shiftCount += 7;
        if (shiftCount < 0) {
        ...
        return roundAndPackFloat32(0, 0x9C - shiftCount, (DI_UINT32) a, status);
    }
}
...
float64 uint64_to_float64(DI_UINT64 a, float_status_t *status)
{
    if (a == 0) return 0;
...
DI_INT32 float32_to_int32(float32 a, float_status_t *status)
{
    DI_UINT32 aSig = extractFloat32Frac(a);
    DI_INT16 aExp = extractFloat32Exp(a);
...
DI_INT32 float32_to_int32_round_to_zero(float32 a, float_status_t *status)
{
    int aSign;
    DI_INT16 aExp;
    DI_UINT32 aSig;
    ...
    if (0 <= shiftCount) {
        ...
        return (DI_INT32)(int32_indefinite);
    }
    else if (aExp <= 0x7E) {
    ...
    z = aSig>>(-shiftCount);
    if ((DI_UINT32) (aSig<<(shiftCount & 31))) {
...
DI_UINT32 float32_to_uint32_round_to_zero(float32 a, float_status_t *status)
{
    int aSign;
    DI_INT16 aExp;
    DI_UINT32 aSig;
...
DI_INT64 float32_to_int64(float32 a, float_status_t *status)
{
    DI_UINT64 aSig64, aSigExtra;
    ...
    if (shiftCount < 0) {
        float_raise(status, float_flag_invalid);
        return (DI_INT64)(int64_indefinite);
    }
    if (aExp) aSig |= 0x00800000;
    else {
        if (get_denormals_are_zeros(status)) aSig = 0;
...
DI_INT64 float32_to_int64_round_to_zero(float32 a, float_status_t *status)
{
    int aSign;
    DI_INT16 aExp;
    DI_UINT32 aSig;
    ...
    aSig64 <<= 40;
    z = aSig64>>(-shiftCount);
    if ((DI_UINT64) (aSig64<<(shiftCount & 63))) {
        float_raise(status, float_flag_inexact);
    }
    if (aSign) z = -z;
    return z;
...
DI_UINT64 float32_to_uint64_round_to_zero(float32 a, float_status_t *status)
{
    DI_UINT64 z=0;
    int aSign;
    DI_INT16 aExp;
...
DI_UINT64 float32_to_uint64(float32 a, float_status_t *status)
{
    int aSign;
    DI_INT16 aExp, shiftCount;
    DI_UINT32 aSig;
    DI_UINT64 aSig64, aSigExtra;
...
DI_UINT32 float32_to_uint32(float32 a, float_status_t *status)
{
    DI_UINT64 val_64 = float32_to_uint64(a, status);
...
float64 float32_to_float64(float32 a, float_status_t *status)
{
    ...
    return packFloat64(aSign, aExp + 0x380, ((DI_UINT64) aSig)<<29);
}
...
float32 float32_round_to_int_with_scale(float32 a, DI_UINT8 scale, float_status_t *status)
{
    float32 z = 0;
    DI_UINT32 lastBitMask, roundBitsMask;
...
float32 float32_frc(float32 a, float_status_t *status)
{
    ...
    DI_UINT32 lastBitMask = 0;
    DI_UINT32 roundBitsMask = 0;
    DI_INT16 aExp = extractFloat32Exp(a);
    DI_UINT32 aSig = extractFloat32Frac(a);
    int aSign = extractFloat32Sign(a);
...
float32 float32_scalef(float32 a, float32 b, float_status_t *status)
{
    ...
    DI_INT16 aExp = extractFloat32Exp(a);
    int aSign = extractFloat32Sign(a);
    DI_UINT32 bSig = extractFloat32Frac(b);
    DI_INT16 bExp = extractFloat32Exp(b);
    ...
    if (bExp == 0xFF) {
        if (bSig) return propagateFloat32NaN_ex(a, b, status);
    }
    ...
    if (aExp == 0xFF) {
        if (aSig) {
            ...
            if (aIsSignalingNaN || bExp != 0xFF || bSig)
                return propagateFloat32NaN_ex(a, b, status);
    ...
    if (bExp <= 0x7E) {
        ...
        if (bSign) {
            if ((DI_UINT32) (bSig<<(shiftCount & 31))) scale++;
            scale = -scale;
        }
...
{
    DI_INT16 aExp, bExp, zExp;
    DI_UINT32 aSig, bSig, zSig;
    DI_INT16 expDiff;
    ...
    if (0 < expDiff) {
        if (aExp == 0xFF) {
            if (aSig) return propagateFloat32NaN_ex(a, b, status);
        ...
        if (aExp == 0xFF) {
            if (aSig | bSig) return propagateFloat32NaN_ex(a, b, status);
            return a;
        }
        if (aExp == 0) {
            zSig = (aSig + bSig) >> 6;
...
float32 float32_mul(float32 a, float32 b, float_status_t *status)
{
    ...
    zSig64 = shift64RightJamming(((DI_UINT64) aSig) * bSig, 32);
    zSig = (DI_UINT32) zSig64;
    if (0 <= (DI_INT32) (zSig<<1)) {
        zSig <<= 1;
        --zExp;
    }
    return roundAndPackFloat32(zSign, zExp, zSig, status);
...
float32 float32_div(float32 a, float32 b, float_status_t *status)
{
    ...
    zSig = (((DI_UINT64) aSig)<<32) / bSig;
    if ((zSig & 0x3F) == 0) {
        zSig |= ((DI_UINT64) bSig * zSig != ((DI_UINT64) aSig)<<32);
    }
    return roundAndPackFloat32(zSign, zExp, zSig, status);
}
...
float32 float32_sqrt(float32 a, float_status_t *status)
{
    ...
    if ((zSig & 0x7F) <= 5) {
        ...
        aSig >>= aExp & 1;
        term = ((DI_UINT64) zSig) * zSig;
        rem = (((DI_UINT64) aSig)<<32) - term;
        while ((DI_INT64) rem < 0) {
            --zSig;
            rem += (((DI_UINT64) zSig)<<1) | 1;
        }
        zSig |= (rem != 0);
    }
...
{
    float_class_t aClass = float_zero;
    float_class_t bClass = float_zero;
    int aSign = 0;
    int bSign = 0;
    ...
    aClass = float32_class(a);
    bClass = float32_class(b);
    ...
    if ((a == b) || ((DI_UINT32) ((a | b)<<1) == 0)) return float_relation_equal;
...
DI_INT32 float64_to_int32(float64 a, float_status_t *status)
{
    DI_UINT64 aSig = extractFloat64Frac(a);
    DI_INT16 aExp = extractFloat64Exp(a);
    int aSign = extractFloat64Sign(a);
    int shiftCount = 0;
    if ((aExp == 0x7FF) && aSig) aSign = 0;
    if (aExp) aSig |= DI_CONST64(0x0010000000000000);
    else {
        if (get_denormals_are_zeros(status)) aSig = 0;
    }
...
DI_INT32 float64_to_int32_round_to_zero(float64 a, float_status_t *status)
{
    int aSign;
    DI_INT16 aExp;
    DI_UINT64 aSig, savedASig;
    DI_INT32 z;
    int shiftCount;
    ...
    aSig |= DI_CONST64(0x0010000000000000);
...
DI_UINT32 float64_to_uint32_round_to_zero(float64 a, float_status_t *status)
{
    DI_UINT64 aSig = extractFloat64Frac(a);
    DI_INT16 aExp = extractFloat64Exp(a);
    int aSign = extractFloat64Sign(a);
    int shiftCount = 0;
    DI_UINT64 savedASig = 0;
...
DI_INT64 float64_to_int64(float64 a, float_status_t *status)
{
    int aSign;
    DI_INT16 aExp;
    DI_UINT64 aSig, aSigExtra;
    int shiftCount = 0;
...
DI_INT64 float64_to_int64_round_to_zero(float64 a, float_status_t *status)
{
    int aSign;
    DI_INT16 aExp;
    DI_UINT64 aSig;
    ...
    if (0 <= shiftCount) {
        if (0x43E <= aExp) {
            if (a != DI_CONST64(0xC3E0000000000000)) {
                float_raise(status, float_flag_invalid);
            }
            return (DI_INT64)(int64_indefinite);
        }
        ...
        z = aSig>>(-shiftCount);
        if ((DI_UINT64) (aSig<<(shiftCount & 63))) {
             float_raise(status, float_flag_inexact);
        }
    }
    if (aSign) z = -z;
...
DI_UINT64 float64_to_uint64_round_to_zero(float64 a, float_status_t *status)
{
    int aSign;
    DI_INT16 aExp;
    DI_UINT64 aSig, z;
...
DI_UINT32 float64_to_uint32(float64 a, float_status_t *status)
{
    DI_UINT64 val_64 = float64_to_uint64(a, status);
...
DI_UINT64 float64_to_uint64(float64 a, float_status_t *status)
{
    int aSign;
    DI_INT16 aExp, shiftCount;
    DI_UINT64 aSig, aSigExtra;
...
float64 float64_round_to_int_with_scale(float64 a, DI_UINT8 scale, float_status_t *status)
{
    float64 z = 0;
    DI_UINT64 lastBitMask, roundBitsMask;
    int roundingMode = get_float_rounding_mode(status);
    DI_INT16 aExp = extractFloat64Exp(a);
    scale &= 0xf;
...
{
    DI_UINT64 lastBitMask = 0;
    DI_UINT64 roundBitsMask = 0;
    int roundingMode = get_float_rounding_mode(status);
    ...
    lastBitMask = DI_CONST64(1) << (0x433 - aExp);
    roundBitsMask = lastBitMask - 1;
...
float64 float64_scalef(float64 a, float64 b, float_status_t *status)
{
    ...
    DI_INT16 aExp = extractFloat64Exp(a);
    int aSign = extractFloat64Sign(a);
    DI_UINT64 bSig = extractFloat64Frac(b);
    DI_INT16 bExp = extractFloat64Exp(b);
    ...
    if (bExp == 0x7FF) {
        if (bSig) return propagateFloat64NaN_ex(a, b, status);
    }

    if (aExp == 0x7FF) {
        if (aSig) {
            int aIsSignalingNaN = (aSig & DI_CONST64(0x0008000000000000)) == 0;
            if (aIsSignalingNaN || bExp != 0x7FF || bSig)
                return propagateFloat64NaN_ex(a, b, status);
    ...
    if (bExp < 0x3FF) {
        ...
        int shiftCount = 0;
        DI_UINT64 savedBSig = 0;

        bSig |= DI_CONST64(0x0010000000000000);
        shiftCount = 0x433 - bExp;
        savedBSig = bSig;
        bSig >>= shiftCount;
        scale = (DI_INT32) bSig;
        if (bSign) {
...
static float64 addFloat64Sigs(float64 a, float64 b, int zSign, float_status_t *status)
{
    ...
    if (0 < expDiff) {
        if (aExp == 0x7FF) {
            if (aSig) return propagateFloat64NaN_ex(a, b, status);
            if (bSig && (bExp == 0)) float_raise(status, float_flag_denormal);
            return a;
        }
        if ((aExp == 0) && aSig)
        ...
        else bSig |= DI_CONST64(0x2000000000000000);
        ...
        else aSig |= DI_CONST64(0x2000000000000000);
        ...
        if (aExp == 0x7FF) {
            if (aSig | bSig) return propagateFloat64NaN_ex(a, b, status);
            return a;
        }
        if (aExp == 0) {
            zSig = (aSig + bSig) >> 9;
        ...
        zSig = DI_CONST64(0x4000000000000000) + aSig + bSig;
        return roundAndPackFloat64(zSign, aExp, zSig, status);
    }
    aSig |= DI_CONST64(0x2000000000000000);
    zSig = (aSig + bSig)<<1;
    --zExp;
    if ((DI_INT64) zSig < 0) {
        zSig = aSig + bSig;
...
static float64 subFloat64Sigs(float64 a, float64 b, int zSign, float_status_t *status)
{
 ...
 bExpBigger:
    ...
    else aSig |= DI_CONST64(0x4000000000000000);

    aSig = shift64RightJamming(aSig, -expDiff);
    bSig |= DI_CONST64(0x4000000000000000);
 bBigger:
    zSig = bSig - aSig;
    zExp = bExp;
    zSign ^= 1;
 ...
 aExpBigger:
    ...
    else bSig |= DI_CONST64(0x4000000000000000);

    bSig = shift64RightJamming(bSig, expDiff);
    aSig |= DI_CONST64(0x4000000000000000);
 aBigger:
    zSig = aSig - bSig;
    zExp = aExp;
 normalizeRoundAndPack:
...
float64 float64_mul(float64 a, float64 b, float_status_t *status)
{
    int aSign, bSign, zSign;
    DI_INT16 aExp, bExp, zExp;
    DI_UINT64 aSig, bSig, zSig0, zSig1;
    ...
    aSig = (aSig | DI_CONST64(0x0010000000000000))<<10;
    bSig = (bSig | DI_CONST64(0x0010000000000000))<<11;
    mul64To128(aSig, bSig, &zSig0, &zSig1);
    zSig0 |= (zSig1 != 0);
    if (0 <= (DI_INT64) (zSig0<<1)) {
        zSig0 <<= 1;
        --zExp;
    }
...
float64 float64_sqrt(float64 a, float_status_t *status)
{
    int aSign;
    DI_INT16 aExp, zExp;
    DI_UINT64 aSig, zSig, doubleZSig;
    DI_UINT64 rem0, rem1, term0, term1;
    ...
    aSig |= DI_CONST64(0x0010000000000000);
    zSig = estimateSqrt32(aExp, (DI_UINT32)(aSig>>21));
    aSig <<= 9 - (aExp & 1);
...
float_class_t float64_class(float64 a)
{
   ...
   if(aExp == 0x7FF) {
       ...
       return (aSig & DI_CONST64(0x0008000000000000)) ? float_QNaN : float_SNaN;
   }
...
int float64_compare(float64 a, float64 b, float_status_t *status)
{
    ...
    aClass = float64_class(a);
    bClass = float64_class(b);
    ...
    if ((a == b) || ((DI_UINT64) ((a | b)<<1) == 0)) return float_relation_equal;
...
int float64_compare_quiet(float64 a, float64 b, float_status_t *status)
{
    int aSign = 0;
    int bSign = 0;
    float_class_t aClass = 0;
    float_class_t bClass = 0;
...
floatx80_t int32_to_floatx80(DI_INT32 a)
{
    int   zSign = 0;
    DI_UINT32 absA = 0;
    int    shiftCount = 0;
...
floatx80_t int64_to_floatx80(DI_INT64 a)
{
    int   zSign = 0;
    DI_UINT64 absA = 0;
    int    shiftCount = 0;
...
floatx80_t float32_to_floatx80(float32 a, float_status_t *status)
{
    DI_UINT32 aSig = extractFloat32Frac(a);
    DI_INT16 aExp = extractFloat32Exp(a);
    int aSign = extractFloat32Sign(a);
    ...
    aSig |= 0x00800000;
    return packFloatx80(aSign, aExp + 0x3F80, ((DI_UINT64) aSig)<<40);
}
...
floatx80_t float64_to_floatx80(float64 a, float_status_t *status)
{
    DI_UINT64 aSig = extractFloat64Frac(a);
    DI_INT16 aExp = extractFloat64Exp(a);
    int aSign = extractFloat64Sign(a);
    ...
    return
        packFloatx80(
            aSign, aExp + 0x3C00, (aSig | DI_CONST64(0x0010000000000000))<<11);
}
...
DI_INT32 floatx80_to_int32(floatx80_t a, float_status_t *status)
{
    ...
    if ((aExp == 0x7FFF) && (DI_UINT64) (aSig<<1)) aSign = 0;
...
DI_INT32 floatx80_to_int32_round_to_zero(floatx80_t a, float_status_t *status)
{
    int aSign;
    DI_INT32 aExp;
    DI_UINT64 aSig, savedASig;
...
DI_INT64 floatx80_to_int64(floatx80_t a, float_status_t *status)
{
    int aSign = 0;
    int shiftCount = 0;
    DI_INT32 aExp;
...
DI_INT64 floatx80_to_int64_round_to_zero(floatx80_t a, float_status_t *status)
{
    int aSign;
    DI_INT32 aExp;
    DI_UINT64 aSig;
    ...
    if (0 <= shiftCount) {
        aSig &= DI_CONST64(0x7FFFFFFFFFFFFFFF);
...
float32 floatx80_to_float32(floatx80_t a, float_status_t *status)
{
    DI_UINT64 aSig = extractFloatx80Frac(a);
    DI_INT32 aExp = extractFloatx80Exp(a);
    int aSign = extractFloatx80Sign(a);
    ...
    return roundAndPackFloat32(aSign, aExp, (DI_UINT32) aSig, status);
}
...
float64 floatx80_to_float64(floatx80_t a, float_status_t *status)
{
    DI_INT32 aExp;
    DI_UINT64 aSig, zSig;
    int aSign;
...
floatx80_t floatx80_round_to_int(floatx80_t a, float_status_t *status)
{
    DI_INT32 aExp = 0;
    DI_UINT64 aSig = 0;
    int aSign;
    ...
    aSig = extractFloatx80Frac(a);
    if (0x403E <= aExp) {
        if ((aExp == 0x7FFF) && (DI_UINT64) (aSig<<1)) {
    ...
    if (aExp < 0x3FFF) {
        ...
        switch (roundingMode) {
         case float_round_nearest_even:
            if ((aExp == 0x3FFE) && (DI_UINT64) (aSig<<1)) {
                set_float_rounding_up(status);
                return packFloatx80(aSign, 0x3FFF, DI_CONST64(0x8000000000000000));
            }
            break;
         case float_round_down:
            if (aSign) {
                set_float_rounding_up(status);
                return packFloatx80(1, 0x3FFF, DI_CONST64(0x8000000000000000));
            }
            else {
         ...
         case float_round_up:
            if (aSign) {
                ...
                set_float_rounding_up(status);
                return packFloatx80(0, 0x3FFF, DI_CONST64(0x8000000000000000));
            }
        }
    ...
    if (z.fraction == 0) {
        z.exp++;
        z.fraction = DI_CONST64(0x8000000000000000);
    }
...
static floatx80_t addFloatx80Sigs(floatx80_t a, floatx80_t b, int zSign, float_status_t *status)
{
    DI_INT32 expDiff;
    DI_INT32 aExp, bExp, zExp;
    DI_UINT64 aSig, bSig, zSig0, zSig1;
    ...
    if (bExp == 0x7FFF) {
        ...
        if (aSig && (aExp == 0)) float_raise(status, float_flag_denormal);
        return packFloatx80(zSign, 0x7FFF, DI_CONST64(0x8000000000000000));
    }
    if (aExp == 0) {
        if (aSig == 0) {
            if ((bExp == 0) && bSig) {
    ...
    zSig0 = aSig + bSig;
    if ((DI_INT64) zSig0 < 0) goto roundAndPack;
 shiftRight1:
    shift64ExtraRightJamming(zSig0, zSig1, 1, &zSig0, &zSig1);
    zSig0 |= DI_CONST64(0x8000000000000000);
    zExp++;
 roundAndPack:
    return
...
static floatx80_t subFloatx80Sigs(floatx80_t a, floatx80_t b, int zSign, float_status_t *status)
{
    DI_INT32 expDiff;
    DI_INT32 aExp, bExp, zExp;
    DI_UINT64 aSig, bSig, zSig0, zSig1;
    ...
    if (aExp == 0x7FFF) {
        if ((DI_UINT64) (aSig<<1)) return propagateFloatx80NaN_ex(a, b, status);
        if (bExp == 0x7FFF) {
            if ((DI_UINT64) (bSig<<1)) return propagateFloatx80NaN_ex(a, b, status);
            float_raise(status, float_flag_invalid);
            return floatx80_default_nan;
    ...
    if (bExp == 0x7FFF) {
        if ((DI_UINT64) (bSig<<1)) return propagateFloatx80NaN_ex(a, b, status);
        if (aSig && (aExp == 0)) float_raise(status, float_flag_denormal);
        return packFloatx80(zSign ^ 1, 0x7FFF, DI_CONST64(0x8000000000000000));
    }
    if (aExp == 0) {
        if (aSig == 0) {
            if (bExp == 0) {
...
floatx80_t floatx80_add(floatx80_t a, floatx80_t b, float_status_t *status)
{
    int aSign = extractFloatx80Sign(a);
...
floatx80_t floatx80_sub(floatx80_t a, floatx80_t b, float_status_t *status)
{
    int aSign = extractFloatx80Sign(a);
    int bSign = extractFloatx80Sign(b);
...
floatx80_t floatx80_mul(floatx80_t a, floatx80_t b, float_status_t *status)
{
    int aSign, bSign, zSign;
    DI_INT32 aExp, bExp, zExp;
    DI_UINT64 aSig, bSig, zSig0, zSig1;
...
floatx80_t floatx80_div(floatx80_t a, floatx80_t b, float_status_t *status)
{
    int aSign, bSign, zSign;
    DI_INT32 aExp, bExp, zExp;
    DI_UINT64 aSig, bSig, zSig0, zSig1;
    DI_UINT64 rem0, rem1, rem2, term0, term1, term2;
    ...
    if ((DI_UINT64) (zSig1<<1) <= 8) {
        mul64To128(bSig, zSig1, &term1, &term2);
        sub128(rem1, 0, term1, term2, &rem1, &rem2);
        while ((DI_INT64) rem1 < 0) {
            --zSig1;
...
floatx80_t floatx80_sqrt(floatx80_t a, float_status_t *status)
{
    int aSign;
    DI_INT32 aExp, zExp;
    DI_UINT64 aSig0, aSig1, zSig0, zSig1, doubleZSig0;
    DI_UINT64 rem0, rem1, rem2, rem3, term0, term1, term2, term3;
    ...
    aSign = extractFloatx80Sign(a);
    if (aExp == 0x7FFF) {
        if ((DI_UINT64) (aSig0<<1)) return propagateFloatx80NaN(a, status);
        if (! aSign) return a;
        float_raise(status, float_flag_invalid);
        return floatx80_default_nan;
    }
    ...
    if ((zSig1 & DI_CONST64(0x3FFFFFFFFFFFFFFF)) <= 5) {
        if (zSig1 == 0) zSig1 = 1;
...
float128_t floatx80_to_float128(floatx80_t a, float_status_t *status)
{
    ...
    return packFloat128_ex(aSign, aExp, zSig0, zSig1);
}
...
floatx80_t float128_to_floatx80(float128_t a, float_status_t *status)
{
    ...
    else aSig0 |= DI_CONST64(0x0001000000000000);
...
{
    DI_INT32 aExp, bExp, zExp;
    DI_UINT64 aSig, bSig0, bSig1, zSig0, zSig1, zSig2;
    int aSign, bSign, zSign;
 ...
 invalid:
    ...
    if (aExp == 0x7FFF) {
        ...
        {
            floatx80_t r = commonNaNToFloatx80(float128ToCommonNaN(b, status));
            return propagateFloatx80NaN_ex(a, r, status);
        }
        if (bExp == 0) {
    ...
    else bSig0 |= DI_CONST64(0x0001000000000000);
...
static float128_t addFloat128Sigs(float128_t a, float128_t b, int zSign, float_status_t *status)
{
    DI_INT32 aExp, bExp, zExp;
    DI_UINT64 aSig0, aSig1, bSig0, bSig1, zSig0, zSig1, zSig2;
    DI_INT32 expDiff;
    ...
    if (0 < expDiff) {
        ...
        if (bExp == 0x7FFF) {
            ...
            return packFloat128_ex(zSign, 0x7FFF, 0, 0);
        }
        if (aExp == 0) ++expDiff;
        else aSig0 |= DI_CONST64(0x0001000000000000);
        ...
        if (aExp == 0) return packFloat128_ex(zSign, 0, zSig0, zSig1);
        zSig2 = 0;
        zSig0 |= DI_CONST64(0x0002000000000000);
        zExp = aExp;
        goto shiftRight1;
    }
    aSig0 |= DI_CONST64(0x0001000000000000);
    add128(aSig0, aSig1, bSig0, bSig1, &zSig0, &zSig1);
    --zExp;
    if (zSig0 < DI_CONST64(0x0002000000000000)) goto roundAndPack;
    ++zExp;
 shiftRight1:
...
static float128_t subFloat128Sigs(float128_t a, float128_t b, int zSign, float_status_t *status)
{
    DI_INT32 aExp, bExp, zExp;
    DI_UINT64 aSig0, aSig1, bSig0, bSig1, zSig0, zSig1;
    DI_INT32 expDiff;
 ...
 bExpBigger:
    if (bExp == 0x7FFF) {
        if (bSig0 | bSig1) return propagateFloat128NaN(a, b, status);
        return packFloat128_ex(zSign ^ 1, 0x7FFF, 0, 0);
    }
    if (aExp == 0) ++expDiff;
    else {
        aSig0 |= DI_CONST64(0x4000000000000000);
    }
    shift128RightJamming(aSig0, aSig1, - expDiff, &aSig0, &aSig1);
    bSig0 |= DI_CONST64(0x4000000000000000);
 bBigger:
...
float128_t float128_mul(float128_t a, float128_t b, float_status_t *status)
{
    int aSign, bSign, zSign;
    DI_INT32 aExp, bExp, zExp;
    DI_UINT64 aSig0, aSig1, bSig0, bSig1, zSig0, zSig1, zSig2, zSig3;
    ...
    if (aExp == 0) {
        if ((aSig0 | aSig1) == 0) return packFloat128_ex(zSign, 0, 0, 0);
        float_raise(status, float_flag_denormal);
        normalizeFloat128Subnormal(aSig0, aSig1, &aExp, &aSig0, &aSig1);
    }
    if (bExp == 0) {
        if ((bSig0 | bSig1) == 0) return packFloat128_ex(zSign, 0, 0, 0);
        float_raise(status, float_flag_denormal);
        normalizeFloat128Subnormal(bSig0, bSig1, &bExp, &bSig0, &bSig1);
    }
    ...
    zSig2 |= (zSig3 != 0);
    if (DI_CONST64(0x0002000000000000) <= zSig0) {
        shift128ExtraRightJamming(zSig0, zSig1, zSig2, 1, &zSig0, &zSig1, &zSig2);
        ++zExp;
    }
    return roundAndPackFloat128(zSign, zExp, zSig0, zSig1, zSig2, status);
...
float128_t float128_div(float128_t a, float128_t b, float_status_t *status)
{
    ...
    if (bExp == 0x7FFF) {
        if (bSig0 | bSig1) return propagateFloat128NaN(a, b, status);
        return packFloat128_ex(zSign, 0, 0, 0);
    }
    if (bExp == 0) {
        if ((bSig0 | bSig1) == 0) {
            if ((aExp | aSig0 | aSig1) == 0) {
    ...
    shortShift128Left(
        aSig0 | DI_CONST64(0x0001000000000000), aSig1, 15, &aSig0, &aSig1);
    shortShift128Left(
        bSig0 | DI_CONST64(0x0001000000000000), bSig1, 15, &bSig0, &bSig1);
...
float128_t int64_to_float128(DI_INT64 a)
{
    ...
    int zSign = 0;
    DI_UINT64 absA = 0;
    DI_UINT8 shiftCount = 0;
    DI_INT32 zExp = 0;


    if (a == 0) return packFloat128_ex(0, 0, 0, 0);
    zSign = (a < 0);
    absA = zSign ? - a : a;
    ...
    return packFloat128_ex(zSign, zExp, zSig0, zSig1);
}
