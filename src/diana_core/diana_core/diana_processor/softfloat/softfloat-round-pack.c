...
DI_INT32 roundAndPackInt32(int zSign, DI_UINT64 exactAbsZ, float_status_t *status)
{
    int roundBits = 0;
    DI_UINT64 absZ = 0;
    DI_INT32 z = 0;
    ...
    roundBits = (int)(exactAbsZ & 0x7F);
    absZ = (exactAbsZ + roundIncrement)>>7;
...
DI_INT64 roundAndPackInt64(int zSign, DI_UINT64 absZ0, DI_UINT64 absZ1, float_status_t *status)
{
    DI_UINT64 exactAbsZ0 = 0;
    DI_INT64 z;
    int roundingMode = get_float_rounding_mode(status);
    int roundNearestEven = (roundingMode == float_round_nearest_even);
    int increment = ((DI_INT64) absZ1 < 0);
    if (! roundNearestEven) {
    ...
    if (increment) {
        ...
        absZ0 &= ~(((DI_UINT64) (absZ1<<1) == 0) & roundNearestEven);
    }
    z = absZ0;
    if (zSign) z = -z;
    if (z && ((z < 0) ^ zSign)) {
...
DI_UINT64 roundAndPackUint64(int zSign, DI_UINT64 absZ0, DI_UINT64 absZ1, float_status_t *status)
{
    int roundingMode = get_float_rounding_mode(status);
    int roundNearestEven = (roundingMode == float_round_nearest_even);
    int increment = ((DI_INT64) absZ1 < 0);
...
void normalizeFloat16Subnormal(DI_UINT16 aSig, DI_INT16 *zExpPtr, DI_UINT16 *zSigPtr)
{
...
float16 roundAndPackFloat16(int zSign, DI_INT16 zExp, DI_UINT16 zSig, float_status_t *status)
{
    DI_INT16 roundIncrement, roundBits, roundMask;
    DI_UINT16 zSigRound = 0;
    int roundingMode = get_float_rounding_mode(status);
    int roundNearestEven = (roundingMode == float_round_nearest_even);
    roundIncrement = 8;
    roundMask = 0xF;
    ...
    if (0x1D <= (DI_UINT16) zExp) {
        if ((0x1D < zExp)
             || ((zExp == 0x1D) && ((DI_INT16) (zSig + roundIncrement) < 0)))
        {
    ...
    zSigRound = ((zSig + roundIncrement) & ~roundMask) >> 4;
...
void normalizeFloat32Subnormal(DI_UINT32 aSig, DI_INT16 *zExpPtr, DI_UINT32 *zSigPtr)
{
...
float32 roundAndPackFloat32(int zSign, DI_INT16 zExp, DI_UINT32 zSig, float_status_t *status)
{
    DI_UINT32 zSigRound = 0;
    DI_INT32 roundIncrement, roundBits;
    const DI_INT32 roundMask = 0x7F;
    ...
    roundBits = zSig & roundMask;
    if (0xFD <= (DI_UINT16) zExp) {
        if ((0xFD < zExp)
             || ((zExp == 0xFD) && ((DI_INT32) (zSig + roundIncrement) < 0)))
        {
            float_raise(status, float_flag_overflow);
            if (roundBits || float_exception_masked(status, float_flag_overflow)) {
                float_raise(status, float_flag_inexact);
    ...
    zSigRound = ((zSig + roundIncrement) & ~roundMask) >> 7;
...
float32 normalizeRoundAndPackFloat32(int zSign, DI_INT16 zExp, DI_UINT32 zSig, float_status_t *status)
{
...
void normalizeFloat64Subnormal(DI_UINT64 aSig, DI_INT16 *zExpPtr, DI_UINT64 *zSigPtr)
{
...
float64 roundAndPackFloat64(int zSign, DI_INT16 zExp, DI_UINT64 zSig, float_status_t *status)
{
    DI_UINT64 zSigRound = 0;
    DI_INT16 roundIncrement, roundBits;
    const DI_INT16 roundMask = 0x3FF;
    int roundingMode = get_float_rounding_mode(status);
    int roundNearestEven = (roundingMode == float_round_nearest_even);
    ...
    roundBits = (DI_INT16)(zSig & roundMask);
    if (0x7FD <= (DI_UINT16) zExp) {
        if ((0x7FD < zExp)
             || ((zExp == 0x7FD)
                  && ((DI_INT64) (zSig + roundIncrement) < 0)))
        {
            float_raise(status, float_flag_overflow);
            if (roundBits || float_exception_masked(status, float_flag_overflow)) {
                float_raise(status, float_flag_inexact);
        ...
        if (zExp < 0) {
            int isTiny = (zExp < -1) || (zSig + roundIncrement < DI_CONST64(0x8000000000000000));
            if (isTiny) {
                if (!float_exception_masked(status, float_flag_underflow)) {
                    float_raise(status, float_flag_underflow);
    ...
    zSigRound = (zSig + roundIncrement)>>10;
...
float64 normalizeRoundAndPackFloat64(int zSign, DI_INT16 zExp, DI_UINT64 zSig, float_status_t *status)
{
...
void normalizeFloatx80Subnormal(DI_UINT64 aSig, DI_INT32 *zExpPtr, DI_UINT64 *zSigPtr)
{
...
floatx80_t SoftFloatRoundAndPackFloatx80(int roundingPrecision,
        int zSign, DI_INT32 zExp, DI_UINT64 zSig0, DI_UINT64 zSig1, float_status_t *status)
{
    DI_UINT64 roundIncrement, roundMask, roundBits;
    int increment;
    DI_UINT64 zSigExact; /* support rounding-up response */

    DI_UINT8 roundingMode = get_float_rounding_mode(status);
    int roundNearestEven = (roundingMode == float_round_nearest_even);
    if (roundingPrecision == 64) {
        roundIncrement = DI_CONST64(0x0000000000000400);
        roundMask = DI_CONST64(0x00000000000007FF);
    }
    else if (roundingPrecision == 32) {
        roundIncrement = DI_CONST64(0x0000008000000000);
        roundMask = DI_CONST64(0x000000FFFFFFFFFF);
    }
 ...
 precision80:
    ...
    if (0x7FFD <= (DI_UINT32) (zExp - 1)) {
        if ((0x7FFE < zExp)
             || ((zExp == 0x7FFE)
                  && (zSig0 == DI_CONST64(0xFFFFFFFFFFFFFFFF))
                  && increment))
        {
            roundMask = 0;
 overflow:
        ...
        if (zExp <= 0) {
            ...
            if (roundNearestEven) increment = ((DI_INT64) zSig1 < 0);
            else {
                if (zSign) {
                    increment = (roundingMode == float_round_down) && zSig1;
                } else {
            ...
            if (increment) {
                zSigExact = zSig0++;
                zSig0 &= ~(((DI_UINT64) (zSig1<<1) == 0) & roundNearestEven);
                if (zSig0 > zSigExact) set_float_rounding_up(status);
                if ((DI_INT64) zSig0 < 0) zExp = 1;
            }
            return packFloatx80(zSign, zExp, zSig0);
...
floatx80_t roundAndPackFloatx80(int roundingPrecision,
        int zSign, DI_INT32 zExp, DI_UINT64 zSig0, DI_UINT64 zSig1, float_status_t *status)
{
    float_status_t round_status = *status;
    floatx80_t result = SoftFloatRoundAndPackFloatx80(roundingPrecision, zSign, zExp, zSig0, zSig1, status);
...
floatx80_t normalizeRoundAndPackFloatx80(int roundingPrecision,
        int zSign, DI_INT32 zExp, DI_UINT64 zSig0, DI_UINT64 zSig1, float_status_t *status)
{
    int shiftCount = 0;
    if (zSig0 == 0) {
...
void normalizeFloat128Subnormal(
     DI_UINT64 aSig0, DI_UINT64 aSig1, DI_INT32 *zExpPtr, DI_UINT64 *zSig0Ptr, DI_UINT64 *zSig1Ptr)
{
    int shiftCount;
...
float128_t roundAndPackFloat128(
     int zSign, DI_INT32 zExp, DI_UINT64 zSig0, DI_UINT64 zSig1, DI_UINT64 zSig2, float_status_t *status)
{
    int increment = ((DI_INT64) zSig2 < 0);
    if (0x7FFD <= (DI_UINT32) zExp) {
        if ((0x7FFD < zExp)
             || ((zExp == 0x7FFD)
                  && eq128(DI_CONST64(0x0001FFFFFFFFFFFF),
                         DI_CONST64(0xFFFFFFFFFFFFFFFF), zSig0, zSig1)
                  && increment))
        {
            float_raise(status, float_flag_overflow | float_flag_inexact);
        ...
        if (zExp < 0) {
            int isTiny = (zExp < -1)
                || ! increment
                || lt128(zSig0, zSig1,
                       DI_CONST64(0x0001FFFFFFFFFFFF),
                       DI_CONST64(0xFFFFFFFFFFFFFFFF));
            shift128ExtraRightJamming(
                zSig0, zSig1, zSig2, -zExp, &zSig0, &zSig1, &zSig2);
            zExp = 0;
            if (isTiny && zSig2) float_raise(status, float_flag_underflow);
            increment = ((DI_INT64) zSig2 < 0);
        }
    }
...
float128_t normalizeRoundAndPackFloat128(
     int zSign, DI_INT32 zExp, DI_UINT64 zSig0, DI_UINT64 zSig1, float_status_t *status)
{
    int shiftCount;
    DI_UINT64 zSig2;
