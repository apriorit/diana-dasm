...
float32 propagateFloat32NaN_ex(float32 a, float32 b, float_status_t *status)
{
    ...
    if (get_float_nan_handling_mode(status) == float_larger_significand_nan) {
      ...
      returnLargerSignificand:
            if ((DI_UINT32) (a<<1) < (DI_UINT32) (b<<1)) return b;
            if ((DI_UINT32) (b<<1) < (DI_UINT32) (a<<1)) return a;
            return (a < b) ? a : b;
        }
        else {
            return b;
...
float64 propagateFloat64NaN_ex(float64 a, float64 b, float_status_t *status)
{
    ...
    if (get_float_nan_handling_mode(status) == float_larger_significand_nan) {
        if (aIsSignalingNaN) {
            ...
            if (bIsSignalingNaN | ! bIsNaN) return a;
      returnLargerSignificand:
            if ((DI_UINT64) (a<<1) < (DI_UINT64) (b<<1)) return b;
            if ((DI_UINT64) (b<<1) < (DI_UINT64) (a<<1)) return a;
            return (a < b) ? a : b;
        }
        else {
            return b;
...
floatx80_t propagateFloatx80NaN_ex(floatx80_t a, floatx80_t b, float_status_t *status)
{
    ...
    b.fraction |= DI_CONST64(0xC000000000000000);
    if (aIsSignalingNaN | bIsSignalingNaN) float_raise(status, float_flag_invalid);
    if (aIsSignalingNaN) {
        if (bIsSignalingNaN) goto returnLargerSignificand;
        return bIsNaN ? b : a;
...
float128_t propagateFloat128NaN(float128_t a, float128_t b, float_status_t *status)
{
    ...
    a.hi |= DI_CONST64(0x0000800000000000);
    b.hi |= DI_CONST64(0x0000800000000000);
    if (aIsSignalingNaN | bIsSignalingNaN) float_raise(status, float_flag_invalid);
    if (aIsSignalingNaN) {
        if (bIsSignalingNaN) goto returnLargerSignificand;
        return bIsNaN ? b : a;
...
floatx80_t floatx80_default_nan;

#endif 

#ifdef FLOAT128

float128_t float128_default_nan;
...
#ifdef FLOATX80
    floatx80_default_nan = packFloatx80(0, floatx80_default_nan_exp, floatx80_default_nan_fraction);
#endif 

#ifdef FLOAT128
    float128_default_nan = packFloat128(float128_default_nan_hi, float128_default_nan_lo);
#endif 
    return 0;
}